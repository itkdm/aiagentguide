...
先给结论：**zero-shot 是不给示例直接做任务，few-shot 是先给少量示例再做任务，Chain-of-Thought 是通过中间推理过程提升复杂推理任务表现。** 

## Zero-shot

Zero-shot 的特点是：不给示例，只给任务说明。  
它适合定义清楚、模型本身已经有足够能力的任务。

优点是简单、便宜；缺点是对任务歧义更敏感。

## Few-shot

Few-shot 会先给几个示例，告诉模型“这个任务应该怎么做”。  
它适合这些场景：
...
Chain-of-Thought 通过显式提示模型进行分步推理，常用于数学、逻辑、多步决策等任务。
