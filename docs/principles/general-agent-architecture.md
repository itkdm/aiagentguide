---
title: 通用 Agent 原理：Agent 架构
description: "从输入、决策、工具、状态和环境反馈等模块出发，拆解 AI Agent 的基本系统架构，并用最小实现说明各部分如何协作。"
summary: 理解 AI Agent 不是一个模型调用，而是由模型、工具、状态和控制逻辑共同组成的运行系统。
keywords:
  - Agent 架构
  - AI Agent 系统架构
  - Agent 核心组件
