---
title: 通用 Agent 原理：核心循环
description: "解释 Agent Loop 如何围绕目标和状态进行决策、调用工具、读取结果、更新状态并判断停止，帮助理解 Agent 的基本执行过程。"
summary: 从一轮轮的决策、行动和观察中理解 Agent 如何持续推进任务，以及循环为什么需要停止条件和失败处理。
keywords:
  - Agent Loop
  - Agent 执行循环
  - AI Agent 工作流程
