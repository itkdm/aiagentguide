---
title: 通用 Agent 原理：多 Agent
description: "解释多 Agent 协作为什么出现，比较单 Agent、监督者模式和多角色协作的取舍，并说明什么时候不应该拆分 Agent。"
summary: 从任务边界、专业分工、并行处理和协作成本出发，判断多 Agent 是必要设计还是徒增复杂度。
keywords:
  - 多 Agent
  - Multi-Agent
  - 多智能体协作
