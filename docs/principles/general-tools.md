---
title: 通用 Agent 原理：工具调用
description: "解释 AI Agent 为什么需要工具，以及模型如何通过结构化 Tool Calling 连接数据库、API、文件和外部环境完成实际动作。"
summary: 从工具定义、参数校验、调用执行到结果回传，理解工具层如何把模型的判断变成可执行能力。
keywords:
  - AI Agent 工具调用
  - Tool Calling
  - Function Calling
