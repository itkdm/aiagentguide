---
title: RAG 教程目录
description: RAG 教程应该怎么学？本栏目按数据准备、检索、生成、评测与生产实践展开，帮助你理解 RAG 系统如何设计、优化和落地。
...
::: warning 必读｜栏目建设状态
目前 RAG 栏目只是初步搭建好了知识架构，正文内容仍然比较简陋，部分页面也存在明显的模板化问题，尚未充分融入真实项目中的设计取舍、工程经验和踩坑总结。现阶段可以参考整个栏目的知识结构，但不建议投入太多时间深入学习本栏目，也不建议把它当作完整、成熟的 RAG 教程使用。后续我们会持续补充和重写，待内容完善到可用标准后，再逐步同步开放。
:::
